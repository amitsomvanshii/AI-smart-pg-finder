...
const CheckInForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/resident/checkin/${bookingId}`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` },
        body
      });
  ...
  const handleDownload = () => {
    window.open(`http://localhost:5000/api/resident/checkin/${bookingId}/download?token=${token}`, '_blank');
  };
