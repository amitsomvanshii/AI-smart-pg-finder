...
const OwnerDashboard = () => {
  ...
  const fetchMyPgs = async () => {
    setLoading(true);
    try {
      const res = await fetch('http://localhost:5000/api/pgs/mine', {
  ...
  const fetchVacancyInsights = async (pgId) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/advanced/pgs/${pgId}/vacancy-forecast`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const fetchRentHistory = async () => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/resident/owner-history', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const handleConfirmRent = async (bookingId) => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/resident/confirm-rent', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleCloseIssue = async (issueId) => {
    setActionLoading(true);
    try {
      const res = await fetch(`http://localhost:5000/api/resident/issue/${issueId}/close`, {
        method: 'PATCH',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleAddFloor = async (e, pgId) => {
    ...
    setFloorError(null);
    try {
      const res = await fetch('http://localhost:5000/api/pgs/floor', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleAddBed = async (e, floorId) => {
    ...
    setBedError(null);
    try {
      const res = await fetch('http://localhost:5000/api/pgs/bed', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleDeleteFloor = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/pgs/floor/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
  ...
  const handleDeleteBed = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/pgs/bed/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
  ...
  const handleUploadPhotos = async (pgId) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/pgs/${pgId}/photos`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` },
        body: formData
      });
  ...
  const handleUploadBedPhotos = async (bedId) => {
    ...
    try {
      const formData = new FormData();
      selectedFiles.forEach(f => formData.append('photos', f));
      const res = await fetch(`http://localhost:5000/api/pgs/bed/${bedId}/photos`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` },
        body: formData
      });
              ...
              <div key={pg.id} className="pg-manage-card">
                ...
                {(pg.photos || []).length > 0 && (
                  <div className="pg-photos-strip">
                    {pg.photos.map((url, i) => (
                      <img key={i} src={`http://localhost:5000${url}`} alt={`PG Photo ${i + 1}`} className="pg-thumb" />
                    ))}
                  </div>
                )}
                              ...
                              return (
                                  ...
                                  {bed.photos && bed.photos.length > 0 && (
                                    <div className="bed-photos-mini">
                                      {bed.photos.map((src, i) => <img key={i} src={`http://localhost:5000${src}`} alt="bed snapshot" />)}
                                    </div>
                                  )}
                                  ...
                                  {bed.status === 'OCCUPIED' && activeBooking && (
                                    <div className="bed-occupant-details">
                                      {activeBooking.passportUrl ? (
                                        <img src={`http://localhost:5000${activeBooking.passportUrl}`} alt="Passport" className="occupant-thumb" />
                                      ) : (
                          ...
                          <tr key={b.id}>
                            <td>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                {b.checkInPhotoUrl ? (
                                  <img 
                                    src={`http://localhost:5000${b.checkInPhotoUrl}`} 
                                    alt="Resident" 
                            ...
                            <td>
                              {b.checkInFormStatus === 'SUBMITTED' && (
                                <button 
                                  className="btn-secondary small-btn"
                                  onClick={() => window.open(`http://localhost:5000/api/resident/checkin/${b.id}/download?token=${token}`, '_blank')}
                                >
