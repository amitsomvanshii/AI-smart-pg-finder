...
const StudentProfile = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        const res = await fetch('http://localhost:5000/api/pgs/my-bookings', {
  ...
  const fetchHubData = async () => {
    setHubLoading(true);
    try {
      const res = await fetch('http://localhost:5000/api/advanced/resident-hub', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleAddExpense = async (e) => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/advanced/expenses', {
        method: 'POST',
        headers: { 
  ...
  const handleSettleSplit = async (expenseId, userId) => {
    try {
      const res = await fetch('http://localhost:5000/api/advanced/expenses/settle', {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const fetchHistory = async () => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/resident/history', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const confirmCancel = async (bookingId) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/pgs/booking/${bookingId}/cancel`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (!res.ok) {
  ...
  const handlePayRent = async (bookingId) => {
    setPayLoading(true);
    try {
      const res = await fetch('http://localhost:5000/api/resident/pay', {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}` 
          ...
          <div className="profile-info">
            ...
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
              ...
              <button 
                ...
                onClick={async () => {
                  await fetch('http://localhost:5000/api/resident/simulate-reminder', {
                    method: 'POST',
