...
const PgDetail = () => {
  ...
  useEffect(() => {
    const fetchPg = async () => {
      try {
        ...
        const res = await fetch(`http://localhost:5000/api/pgs/${id}/layout`, {
  ...
  useEffect(() => {
    ...
    const fetchPrediction = async () => {
      setPredLoading(true);
      try {
        const res = await fetch(`http://localhost:5000/api/advanced/predict/${id}`);
        if (res.ok) {
          const data = await res.json();
          setPrediction(data);
        }
  ...
  useEffect(() => {
    ...
    const fetchMyLifestyle = async () => {
      try {
        const res = await fetch('http://localhost:5000/api/advanced/lifestyle', {
  ...
  useEffect(() => {
    ...
    const fetchCanReview = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/advanced/pgs/${id}/can-review`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (res.ok) {
          const data = await res.json();
  ...
  useEffect(() => {
    ...
    const fetchInsights = async () => {
      ...
      try {
        const res = await fetch(`http://localhost:5000/api/advanced/pgs/${id}/review-insights`);
        if (res.ok) {
          const data = await res.json();
  ...
  const submitBooking = async (e) => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:5000/api/pgs/book-bed', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` },
        body: formData
      });
      ...
      // Fetch QR Code
      const qrRes = await fetch(`http://localhost:5000/api/advanced/booking/${data.booking.id}/qr`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleReviewSubmit = async (e) => {
    ...
    setReviewError(null);
    try {
      const res = await fetch(`http://localhost:5000/api/advanced/pgs/${id}/review`, {
        method: 'POST',
      ...
      const pgRes = await fetch(`http://localhost:5000/api/pgs/${id}/layout`, {
    ...
    <div className="detail-container fade-in">
      ...
      {pg.photos && pg.photos.length > 0 && (
        <div className="pg-detail-gallery">
          <img
            src={`http://localhost:5000${pg.photos[0]}`}
            alt={pg.name}
          ...
          {pg.photos.length > 1 && (
            <div className="gallery-thumbs">
              {pg.photos.slice(1, 5).map((url, i) => (
                <img key={i} src={`http://localhost:5000${url}`} alt={`Photo ${i + 2}`} className="gallery-thumb" />
              ))}
              {pg.photos.length > 5 && (
                      ...
                      >
                        ...
                        {bed.photos && bed.photos.length > 0 && (
                          <div className="pg-bed-photos-preview">
                            <img src={`http://localhost:5000${bed.photos[0]}`} alt="Bed" />
                          </div>
                        )}
          ...
          <div className="modal-content fade-in" onClick={e => e.stopPropagation()}>
            {!bookingSuccessData ? (
              ...
              <div className="booking-success-view fade-in">
                ...
                <a 
                  href={`http://localhost:5000/api/advanced/booking/${bookingSuccessData.bookingId}/agreement?token=${token}`} 
                  target="_blank" 
