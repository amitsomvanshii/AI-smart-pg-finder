...
const AdminDashboard = () => {
  ...
  const fetchGlobalData = async () => {
    ...
    try {
      const [pgsRes, statsRes, leadsRes, usersRes, analyticsRes] = await Promise.all([
        fetch('http://localhost:5000/api/admin/flagged', { headers: { 'Authorization': `Bearer ${token}` } }),
        ...
        fetch(`http://localhost:5000/api/admin/analytics?range=${timeRange}`, { headers: { 'Authorization': `Bearer ${token}` } })
      ]);
  ...
  const handleVerifyPg = async (pgId) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/admin/verify/${pgId}`, {
        method: 'POST',
  ...
  const handleDeleteLead = async (leadId) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/admin/lead/${leadId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
